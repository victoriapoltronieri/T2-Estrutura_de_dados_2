...
void quicksort(Suffix** vet, int inicio, int fim){
    ...
    printf("init:   %d      end:    %d", init, end);
    printf("oi aqui dentro\n");
    if(inicio < fim){
        printf("oi dentro do if\n");
        while(init < end){
            comp = 0;
            printf("dentro do while\n");
            printf("comp:   %d      init:       %d      end:    %d", comp, init, end);
            while(comp <= 0 && init < end){ // pivot é maior 
                int index1 = esq->index;
                int index2 = pivot->index;
                comp = strcmp(esq->s->c + index1, pivot->s->c + index2);
                printf("dentro do outro while 1         comp:   %d\n", comp);
                if(comp <= 0){
                    esq[init++];
                }
            }
            ...
            while(comp > 0 && init < end){
                ...
                int index2 = pivot->index;
                comp = strcmp(dir->s->c + index1, pivot->s->c + index2);
                if(comp > 0){
                    dir[end--];
                }
            }
